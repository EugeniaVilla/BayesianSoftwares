...
setwd("C:/Users/Utente/Politecnico di Milano/V ANNO/BAYESIAN/progetto/codici/Linear Model")
...
linearCode <- nimbleCode({
  ...
  for(i in 1:k){    #with NIMBLE, if we want to define a vector inside the model, we have to do a for cicle!
    beta0[i] <- 0 # beta0 = [0 0 0 0]
  }
  sigma <- pow(sigmasq,0.5)
  sigmasq ~ dinvgamma(nu0,nu0*sigma0/2)
})

linearConsts <- list(c=c,n=N,k=p,nu0=nu0,sigma0=sigma0)
linearInits <- list( beta=b0, sigmasq=sigma2, tau = 1/sigma2, Sigma=Sigma_beta)
...
{tic()
  ...
  Clinear <- compileNimble(linear)
  linearConf <- configureMCMC(linear, print = TRUE)
  linearConf$addMonitors(c("tau", "sigmasq", "beta","Sigma"))
  
  linearMCMC <- buildMCMC(linearConf)
  ClinearMCMC <- compileNimble(linearMCMC, project = linear)
  set.seed(1)
  {tic()
    Psamples <- runMCMC(ClinearMCMC, niter=75000, nburnin=25000, thin=10, nchains=4,samplesAsCodaMCMC = TRUE,summary=TRUE)
    toc()}
  toc()}

# mcmc.out <- nimbleMCMC(code = linearCode, constants = linearConsts, data = linearData, inits = linearInits,
...
model_NC<-
  ...
  sigmasquared ~ dunif(0,10000) # stochastic node
